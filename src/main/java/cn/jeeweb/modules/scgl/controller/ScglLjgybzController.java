...
public class ScglLjgybzController extends BaseCRUDController<ScglLjgybz, String> {
    ...
    public PageJson<ScjhglLjgl> ajaxlbjglList(Queryable queryable, ScjhglLjgl scjhglLjgl, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        List<ScjhglLjgl> results = pageJson.getResults();
        for (ScjhglLjgl s : results) {
            String ljid = s.getId();
            EntityWrapper<ScglGydlbz> wrapper = new EntityWrapper<>();
            wrapper.eq("LJID", ljid);
            List<ScglGydlbz> scglGydlbzs = scglGydlbzService.selectList(wrapper);
            String dls = "";
            for (ScglGydlbz scglGydlbz : scglGydlbzs) {
                if (dls == "") {
                    dls = scglGydlbz.getGydlmc();
                } else {
                    dls = dls + "," + scglGydlbz.getGydlmc();
                }
            }
            s.setDls(dls);
        }
