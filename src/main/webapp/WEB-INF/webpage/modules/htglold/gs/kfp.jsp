...
<h2>${htglHt.htmc}</h2>
<grid:grid id="Htmx" url="${adminPath}/htgl/gs/ajaxHtmxList?id=${htglHt.id}&type=1" pageable="true">
    ...
    <grid:button title="删除" groupname="opt" function="deleteFp"
    outclass="btn-danger" url="${adminPath}/htgl/gs/deleteFp?id=\"+row.id+\"&htid=${htglHt.id}" />
...
<script type="text/javascript">
    ...
    //详情
    function createFp(title, url, gridId, id, width, height, tipMsg){
        url = "${adminPath}/htglold/gs/createFp?htid=${htglHt.id}";
        openDia("开发票",url,gridId,"40%","30%");
    }
        ...
        <%--if (idsArray.length>0){--%>
            ...
            <%--layer.confirm('是否要删除该合同!', {--%>
                ...
                <%--}, function (index, layero) {--%>
                    <%--$.ajax({--%>
                        <%--type: "GET",--%>
                        <%--url: "${adminPath}/htglold/gs/deleteFp?ids="+ids,--%>
                        <%--success: function (data) {--%>
                            <%--refreshTable2(gridId);--%>
                        <%--}--%>
                    <%--});--%>
