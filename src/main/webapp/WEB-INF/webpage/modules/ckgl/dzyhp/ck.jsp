...
<input type="hidden" id="dzyhpid" name="dzyhpid" value="${ckglDzyhp.id}">
<input type="hidden" id="kc" name="kc" value="${ckglDzyhp.kc}">
<h2> ${ckglDzyhp.gg} </h2>
<h4>库存：${ckglDzyhp.kc}</h4>
<hr>
<div class="row">
    <div class="col-md-3">
...
<script type="text/javascript">
    ...
    //点击保存，保存数据
    function check() {
        var dzyhpid = $("#dzyhpid").val();
        var ly = $("#ly").val();
        var cksl = $("#cksl").val();
        $.ajax({
            type: "GET",
            url: "${adminPath}/ckgl/dzyhp/saveDzyhpkcck",
            data: {
                dzyhpid: dzyhpid,
                ly: ly,
                cksl: cksl
