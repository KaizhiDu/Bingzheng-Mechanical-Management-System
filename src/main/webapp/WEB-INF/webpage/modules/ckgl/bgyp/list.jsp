...
<hr>
<div class="row">
    <div id="BgypGridQuery" class="col-md-12">
...
<grid:grid id="Bgyp"
           url="${adminPath}/ckgl/bgyp/ajaxBgypList" pageable="true">
    ...
    <grid:button title="入库" groupname="opt" function="rk"
                 outclass="btn-success" url="${adminPath}/ckgl/bgyp/rk?id=\"+row.id+\"" />
    <grid:button title="出库" groupname="opt" function="ck"
                 outclass="btn-primary" url="${adminPath}/ckgl/bgyp/ck?id=\"+row.id+\"" />
    <grid:button title="查看进销详情" groupname="opt" function="ckxq"
                 outclass="btn-warning" url="${adminPath}/ckgl/bgyp/ckxq?id=\"+row.id+\"" />
    ...
    <grid:toolbar function="createBgyp" icon="fa fa-plus" btnclass="btn btn-sm btn-primary" title="添加办公用品"/>
...
<script type="text/javascript">
    //添加标准件
    function createBgyp(title, url, gridId, id, width, height, tipMsg) {
        var url = "${adminPath}/ckgl/bgyp/createBgyp";
