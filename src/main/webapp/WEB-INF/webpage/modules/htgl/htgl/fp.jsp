...
<div class="row">
    <div id="fpxqGridQuery" class="col-md-12">
...
<div>
    <h2>剩余发票：<span id="fpje" name="fpje"></span></h2>
</div>

<input type="hidden" id="htid" name="htid" value="${htid}">

<grid:grid id="fpxq" url="${adminPath}/htgl/htgl/ajaxFpxqList?htid=${htid}" pageable="true">
    ...
    <grid:column label="发票金额" name="fpje"/>
    <grid:column label="备注" name="bz"/>
    <grid:toolbar function="kfp" btnclass="btn btn-sm btn-primary" title="开发票"/>
    <grid:toolbar function="deleteFp" btnclass="btn btn-sm btn-danger" title="删除"/>
...
<script type="text/javascript">
    ...
    function getFpje() {
        ...
        $.ajax({
            type: "GET",
            url: "${adminPath}/htgl/htgl/getFpje?htid=" + htid,
    ...
    function kfp(title, url, gridId, id, width, height, tipMsg){
        var htid = $("#htid").val();
        url = "${adminPath}/htgl/htgl/kfp?htid=" + htid;
        openDia("开发票",url,gridId,"70%","70%");
    }
    ...
    function deleteFp(title, url, gridId, id, width, height, tipMsg){
        var htid = $("#htid").val();
        //获取选中行的id数组
        var idsArray = $("#fpxqGrid").jqGrid("getGridParam", "selarrrow");
        if (idsArray.length>0){
            ...
            layer.confirm('是否要删除!', {
                    ...
                    $.ajax({
                        type: "GET",
                        url: "${adminPath}/htgl/htgl/deleteFp?ids="+ids+"&htid="+htid,
                        success: function (data) {
                            getFpje();
                            var queryParams = {};
